...
pub fn get_args() -> MyResult<Config> {
    ...
    let delimiter = matches.value_of_lossy("delimiter").unwrap();
        ...
        match open(filename) {
            ...
            Ok(mut file) => {
                match &config.extract {
                    Chars(char_pos) => {
                        file.lines().map(|l| l.unwrap()).for_each(|line| {
                    ...
                    Fields(field_pos) => {
                        ...
                        let delimiter = &String::from_utf8(vec![config.delimiter])?;

                        println!("{}", extract_fields(reader.headers()?, &field_pos).join(&delimiter));

                        for record in reader.records() {
                            println!("{}", extract_fields(&record?, &field_pos).join(&delimiter));
                        }
...
fn extract_fields(record: &StringRecord, field_pos: &[Range<usize>]) -> Vec<String> {
    field_pos
        .iter()
        .cloned()
        .flat_map(|range| range.filter_map(|i| record.get(i)).map(|s| s.to_string()))
        .collect()
}
